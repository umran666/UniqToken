...
def test_projection_is_training_only_lower_bound_and_three_seed():
    ...
    feasibility = {"records": [{"tokenizer": name, "training_steps": 1000}
                               for name in phase_c.NAMES]}
    result = calibration.projected_lower_bound(records, feasibility, 2.0)
    expected_seconds = len(phase_c.NAMES) * len(phase_c.SEEDS) * (2 + 300 + 1000)
    assert result["lower_bound_wall_seconds"] == expected_seconds
    assert result["lower_bound_cost_usd"] == pytest.approx(expected_seconds / 1800)
...
def test_projection_refuses_incomplete_or_uncapped_records(bad_records):
    ...
    with pytest.raises((ValueError, KeyError)):
        calibration.projected_lower_bound(bad_records, feasibility, 1.0)
...
def test_run_uses_training_prefix_only_and_never_scores_validation(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(calibration, "runtime_fingerprint", lambda identity: {"identity": identity})
    monkeypatch.setattr(h, "load_tokenizer", lambda source, root: source["tokenizer"])
    seen = []

    def fake_measure(tok, texts, seed):
        seen.append((tok, texts, seed))
    ...
    assert len(seen) == 3
    assert all(texts == ["a" * 600_000] and seed == 0 for _, texts, seed in seen)
