...
def test_calibration_plan_reads_only_selected_prefix_not_phase_c_validation(monkeypatch, tmp_path):
    ...
    frozen.mkdir()
    exposure = {"status": "phase_c_exposure_frozen", "normalized_utf8_bytes": phase_c.EXPOSURE_BYTES,
                "test_split_opened": False,
                "ordered_documents": [{"train_row_index": i, "normalized_utf8_bytes": 1, "position": p}
                                      for p, i in enumerate((0, 2))]}
    exposure["content_sha256"] = h.digest(exposure)
    h.write_new_json_atomic(frozen / "exposure.json", exposure)
    h.write_new_json_atomic(tmp_path / "receipt.json", {
        "status": "phase_c_exposure_frozen", "exposure_sha256": "hash",
        "confirmation_validation_sha256": "hash", "test_split_opened": False})
    ...
    monkeypatch.setattr(phase_b, "check_selection", lambda *a: {
        "phase_a_identity": {},
    ...
    monkeypatch.setattr(exposure_gate, "document", lambda row, index: {
        "train_row_index": index, "normalized_utf8_bytes": 1})
    args = SimpleNamespace(phase_a=tmp_path / "phase-a", dataset=tmp_path / "dataset",
                           selection=tmp_path / "selection", selection_sha256="hash", device="cuda",
                           protocol=tmp_path / "protocol", phase_b_report=tmp_path / "report",
                           phase_b_ledger=tmp_path / "ledger", source_manifest=source_dir / "manifest.json",
                           exposure_manifest=frozen / "exposure.json", exposure_manifest_sha256="hash",
...
@pytest.mark.parametrize("device,expected_gpu,gpu_name,memory", [
...
def test_runtime_rejects_wrong_gpu_or_memory(monkeypatch):
    ...
    monkeypatch.setattr(calibration.platform, "python_version", lambda: "3.10.17")
    monkeypatch.setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8")
    monkeypatch.setattr(calibration.subprocess, "run", lambda *a, **k:
                        SimpleNamespace(stdout="NVIDIA A100-SXM4-80GB, 580.95.05, 81251 MiB"))
    monkeypatch.setitem(sys.modules, "torch", SimpleNamespace(
        __version__="2.6.0+cu124", version=SimpleNamespace(cuda="12.4"),
...
def test_run_uses_training_prefix_only_and_never_scores_validation(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(calibration, "runtime_fingerprint", lambda identity, device, expected_gpu:
                        {"identity": identity, "device": device, "expected_gpu": expected_gpu})
    ...
    args = SimpleNamespace(device="cuda", expected_gpu="T4", feasibility_receipt=receipt_path,
                           phase_a=tmp_path / "phase-a" / "ledger.json", hourly_rate=1.5,
                           max_wall_seconds=480, max_compute_cost_usd=0.35,
                           modal_hard_timeout_seconds=540,
                           price_source="https://modal.com/pricing", price_observed_date="2026-09-20",
...
def test_run_rejects_unsafe_compute_cap_before_data_access(monkeypatch):
    ...
    args = SimpleNamespace(hourly_rate=1.0, max_wall_seconds=481,
                           max_compute_cost_usd=0.35, modal_hard_timeout_seconds=540)
