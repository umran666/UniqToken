...
def validate_plans(old, current):
    research.require(old.get("stage") == current.get("stage") == "A-SCREEN", "migration only supports A-SCREEN; no Phase B/C")
    research.require(old.get("result_label") == "SCREENING", "screening label required")
    research.require({k: v for k, v in old.items() if k != "identity"} ==
                     {k: v for k, v in current.items() if k != "identity"}, "dataset/configuration/assignment plan mismatch")
    trained, active = old["identity"], current["identity"]
    research.require(not trained["working_tree_dirty"] and not active["working_tree_dirty"], "clean committed code required")
    research.require(trained.get("extension_hash") and trained["extension_hash"] == active.get("extension_hash"), "extension mismatch")
...
def validate_migrated(row, expected, identity, source, training, validation, output):
    m = row["migration"]
    research.require(m.get("schema_version") == VERSION and m.get("status") == "condition_revalidated", "invalid migration status/version")
    ...
    old = m["original_plan"]
    current = stages._stage_plan("A-SCREEN", identity, source, training, validation,
                                 [(n, v) for n in research.COHORT for v in research.VOCABS])
    proof = validate_plans(old, current)
    research.require(m.get("code_approval") == proof and m.get("trained_commit") == old["identity"]["commit_hash"]
                     and m.get("revalidated_commit") == identity["commit_hash"], "migration commit evidence mismatch")
    ...
    research.require(research.file_hash(original_path) == m["original_condition_sha256"], "original condition hash mismatch")
    plan_path = Path(output) / "originals" / "plan.json"
    research.require(research.file_hash(plan_path) == m["original_plan_sha256"] and research.read_json(plan_path) == old,
                     "original plan evidence mismatch")
    envelope = research.read_json(original_path)
    original = envelope["record"]
    research.require(envelope.get("status") == "condition_complete" and "migration" not in original, "no chained migration")
    ...
    research.require(type(fallback) is int and type(tokens) is int and tokens > 0 and 0 <= fallback <= tokens
                     and metric["byte_fallback_percent"] == 100.0 * fallback / tokens, "invalid revalidated fallback metric")
    research.require({k: v for k, v in row.items() if k not in ("migration", "validation")} ==
                     {k: v for k, v in original.items() if k != "validation"}, "original training provenance changed")
...
def migrate(args):
    ...
    for path in paths:
        ...
        row = envelope["record"]
        research.require(row.get("artifact") == f"{conditions[index][0]}-{conditions[index][1]}", "condition artifact assignment mismatch")
        research.require((source_dir / row["artifact"]).is_dir() and
                         (source_dir / row["artifact"]).resolve().is_relative_to(source_dir), "missing or escaping artifact")
        research.require(envelope.get("status") == "condition_complete" and "migration" not in row, "no chained migration")
        research.require(row["tokenizer"] in SUPPORTED, "tokenizer not approved for migration")
        stages._validate_condition(row, conditions[index], old["identity"], plan["dataset"],
                                   plan["training"], plan["validation"], source_dir)
        checked.append((path, index, envelope))
    if args.dry_run:
        return {"dry_run": True, "eligible_conditions": len(checked), "code_approval": proof,
                "metrics_recomputed": False, "output_written": False}
    ...
    for path, index, envelope in checked:
        ...
        research.require(research.artifact_hashes(output / artifact) == original["artifact_hashes"], "copied artifact mismatch")
        tokenizer = research.load_tokenizer(original, output)
        row = copy.deepcopy(original)
        row["validation"] = stages.validation_metric(tokenizer, validation_texts, plan["validation"]["source_utf8_bytes"])
        row["migration"] = {
            "schema_version": VERSION, "status": "condition_revalidated",
            "trained_commit": original["git_commit"], "revalidated_commit": plan["identity"]["commit_hash"],
            "revalidated_identity": plan["identity"], "revalidation_timestamp": datetime.now(timezone.utc).isoformat(),
            "code_approval": proof, "original_plan": old,
            "original_plan_sha256": research.file_hash(output / "originals" / "plan.json"),
            "original_condition_file": path.name, "original_condition_sha256": research.file_hash(path),
        }
        stages._validate_condition(row, conditions[index], plan["identity"], plan["dataset"],
                                   plan["training"], plan["validation"], output)
