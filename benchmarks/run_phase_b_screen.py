...
def implementation_hash():
    ...
    for directory, suffix in (("uniqtoken", ".py"), ("benchmarks", ".py"), ("crates", ".rs")):
        files.extend(
            ...
            for p in (h.ROOT / directory).rglob("*" + suffix)
    ...
    return h.digest(
        {name: lineage.git("hash-object", f"--path={name}", "--", name).decode().strip() for name in sorted(files)}
    )
...
def dataset_context(path):
    ...
    byte_rows = {
        "train": [
            {h.BYTE_BUDGET_FIELD: len(text.encode("utf-8")), h.BYTE_AUDIT_FIELD: raw[h.digest(text)]} for text in train
        ],
        "validation": [
    ...
    training = {
        "scope": "representative_stratified_screen",
        "normalized_utf8_bytes": info["actual_normalized_utf8_bytes"],
        "source_utf8_bytes": info["source_utf8_bytes"],
        "documents": len(train),
        "assignment_hash": info["assignment_hash"],
        "screen_selection": info,
    }
    evaluation = {
...
def frozen_exposure_context(args, selection):
    ...
    sample = exposure_gate.check_exposure(exposure, receipt, args.selection_sha256, args.source_manifest_sha256)
...
def verify_phase_a(path, dataset_path):
    ...
    for row in ledger["records"]:
        h.require(
            row["artifact_hashes"] and row["artifact"] == f"{row['tokenizer']}-{row['vocab_budget']}",
...
def selection_payload(ledger, ledger_hash, implementation_hash):
    meta = ledger["metadata"]
    h.require(
        meta["stage"] == "A-SCREEN" and meta["result_label"] == "SCREENING" and meta["status"] == "complete",
        "complete A-SCREEN required",
    )
    rows = ledger["records"]
    h.require(
        len(rows) == 15
        and {(r["tokenizer"], r["vocab_budget"]) for r in rows} == {(n, v) for n in h.COHORT for v in h.VOCABS},
        "incomplete Phase A cohort",
    )
        ...
        selected.append(
            {
                "tokenizer": name,
                "vocab_budget": vocab,
                "actual_vocab_size": row["actual_vocab_size"],
                "artifact": row["artifact"],
                "artifact_hashes": row["artifact_hashes"],
                "tokenizer_config": row["tokenizer_config"],
                "trained_commit": row["git_commit"],
                "extension_hash": row["extension_hash"],
    ...
    body = {
        "selection_schema_version": VERSION,
        "policy": POLICY,
        "phase": "B-SCREEN",
        "result_label": "SCREENING",
        "test_metrics_used": False,
        "lm_results_used": False,
        "phase_a_ledger_sha256": ledger_hash,
        ...
        "phase_a_validation_evidence": [
            {
                ...
                "vocab_budget": r["vocab_budget"],
                **{key: r["validation"][key] for key in METRICS},
            }
            for r in rows
        ],
        "implementation_git_blob_hash": implementation_hash,
        "model_config_cpu_template": h.model_config("B", "cpu"),
        "byte_budget_field": h.BYTE_BUDGET_FIELD,
        "byte_audit_field": h.BYTE_AUDIT_FIELD,
...
def freeze(args):
    ...
    return {
        "selection": str(output.resolve()),
        "sha256": h.file_hash(output),
        "content_sha256": selection["content_sha256"],
        "tokenizer_conditions": 9,
        "lm_runs": 18,
    }
...
def check_selection(path, expected_hash, ledger, ledger_path, identity):
    ...
    h.require(
        selection_implementation in {FROZEN_SELECTION_IMPLEMENTATION_HASH, implementation_hash()},
        "selection/configuration/implementation mismatch: unrecognized selection implementation",
    )
    expected = selection_payload(ledger, h.file_hash(ledger_path), selection_implementation)
...
def check_runtime(identity, historical, device="cpu"):
    ...
    h.require(
        identity["extension_status"] == "installed" and identity["extension_hash"] == historical["extension_hash"],
        "Phase A tokenizer extension required (use the pinned Linux build)",
    )
    for package in ("python", "sentencepiece", "numpy", "regex"):
        h.require(
            identity["versions"][package] == historical["versions"][package],
            f"tokenizer environment mismatch: {package}",
        )
...
def prepare(args, *, execution=False):
    ...
    h.require(
        args.flops >= 100 * h.training_flops(max(h.VOCABS), h.SCREEN, 1),
    ...
    plan = {
        **identity,
        "identity": identity,
        "phase_b_schema_version": VERSION,
        "stage": "B-SCREEN",
        "result_label": "SCREENING",
        "status": "planned",
        ...
        **exposure,
        ...
        "byte_matched_documents": count,
        "byte_matched_exposure": h.byte_totals(byte_rows["train"], count),
        "byte_matched_assignment_hash": h.digest(
            [h.digest(docs["train"][i % len(docs["train"])]) for i in range(count)]
        ),
    }
...
def validate_result(row, expected, plan, source, byte_rows):
    ...
    required = {
        ...
        "special_tokens": h.SPECIAL_IDS,
        "requested_budget": plan["budgets"][regime],
    ...
    h.require(
        type(targets) is int
        and type(squares) is int
        and targets > 0
        and targets <= squares <= targets * plan["model_config"]["context"]
        and row["training_steps"] > 0,
        "invalid/no-op training accounting",
    )
    h.require(
        type(row.get("fully_predicted_documents")) is int and row["fully_predicted_documents"] >= 1,
        "FLOP-matched run predicted no complete document",
    )
    h.require(
        all(row.get(k) == v for k, v in h.flop_accounting(vocab, cfg, targets, squares).items()),
        "FLOP accounting mismatch",
    )
    ...
    h.require(
        row["training_bytes"] == exposure and row["completed_document_bytes"] == exposure[h.BYTE_BUDGET_FIELD],
        "source/normalized exposure mismatch",
    )
    if regime == "bytes":
        h.require(
            complete == plan["byte_matched_documents"] and exposure == plan["byte_matched_exposure"],
            "byte matching mismatch",
        )
    else:
        h.require(
            0.99 * plan["budgets"][regime] <= row["actual_analytical_flops"] <= plan["budgets"][regime],
            "FLOP matching mismatch",
        )
    ...
    h.require(
        ...
        == h.nll_metrics(
            ...
            expected_targets,
            validation["normalized_utf8_bytes"],
            source_utf8_bytes=validation["source_utf8_bytes"],
        ),
        "held-out CE/NLL/BPB mismatch",
    )
...
def check_execution_lock(args, output, plan):
    ...
    if plan["model_config"]["device"].startswith("cuda"):
        h.require(
            os.environ.get("CUBLAS_WORKSPACE_CONFIG") == plan["cuda_workspace_config"], "CUDA configuration changed"
        )
...
def train_phase_b_condition(tok, docs, regime, budget, device, byte_rows):
    ...
    measured = h.train_lm(tok, docs, h.SCREEN, 128, regime, budget, 0, device, False, document_bytes=byte_rows)
...
def load_resume_records(args, output, plan, ledger, byte_rows):
    ...
    for path in sorted(output.glob("condition-*.json")):
        ...
        source = next(
            r for r in ledger["records"] if (r["tokenizer"], r["vocab_budget"]) == tuple(plan["conditions"][index][:2])
        )
        h.require(
...
def run(args):
    ...
    if output.exists():
        h.require(
            resume and output.is_dir(), "output exists; use --resume for a provenance-matched partial Phase B run"
        )
