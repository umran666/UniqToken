"""Short L4 calibration; no Phase C condition or validation result is written."""
...
app = modal.App("uniqtoken-phase-c-gpu-calibration-v1")
...
results = modal.Volume.from_name("uniqtoken-phase-c-gpu-calibration-v1", create_if_missing=True)
...
image = (
    ...
    .env({"PATH": "/root/.cargo/bin:/usr/local/bin:/usr/bin:/bin", "PYTHONUNBUFFERED": "1",
          ...
          "OMP_NUM_THREADS": "2", "MKL_NUM_THREADS": "2", "RAYON_NUM_THREADS": "1"})
    ...
    .add_local_file(HERE / "repo-phase-c-calibration.bundle", "/tmp/repo.bundle", copy=True)
    .run_commands(
...
def arguments(output):
    return [
        ...
        "--device", "cuda", "--hourly-rate", "1.115712", "--output", str(output),
    ]


@app.function(
    image=image, gpu="L4", cpu=4, memory=16384, timeout=900, max_containers=1, retries=0,
...
def calibration():
    ...
    local = Path("/tmp/phase-c-calibration-v1/receipt.json")
    ...
    if process.returncode:
        ...
        raise RuntimeError(f"calibration failed with exit {process.returncode}")
    ...
    if receipt["experiments_started"] or receipt["validation_scored"] or receipt["test_split_opened"]:
        raise RuntimeError("calibration scope violation")
    target = Path("/results/phase-c-gpu-calibration-v1/receipt.json")
