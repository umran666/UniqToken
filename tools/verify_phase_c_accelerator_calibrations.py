...
def verify_one(gpu, measurement_path, billing_path, feasibility, commit):
    ...
    require(measurement["status"] == "CALIBRATION_COMPLETE_COST_GATE_NOT_CLEARED"
            and measurement["purpose"] == "non_experimental_training_stack_calibration"
            and measurement["device"] == "cuda" and measurement["experiments_started"] is False
            and measurement["validation_scored"] is False and measurement["test_split_opened"] is False,
            "calibration scope violated")
    require(measurement["runtime"]["identity"]["commit_hash"] == commit
            and measurement["runtime"]["identity"]["working_tree_dirty"] is False,
            "calibration commit or tree mismatch")
    require(measurement["runtime"]["identity"]["extension_hash"] == EXTENSION_HASH
    ...
    gpu_name_ok = "T4" in runtime["gpu_name"] if gpu == "T4" else (
        "A100" in runtime["gpu_name"] and "40GB" in runtime["gpu_name"])
    require(runtime["expected_gpu"] == gpu and gpu_name_ok
    ...
    require((14_000 <= memory <= 17_000) if gpu == "T4" else (38_000 <= memory <= 42_000),
            "GPU memory class mismatch")
    pins = measurement["provenance"]
    require(pins["exposure_manifest_sha256"] == EXPOSURE_HASH
    ...
    require(measurement["sample_normalized_utf8_bytes"] == 999_753
    ...
    require(limits == {"max_wall_seconds": 480.0, "max_compute_cost_usd": 0.35,
                       "modal_hard_timeout_seconds": 540}
            and measurement["wall_seconds"] < limits["max_wall_seconds"],
            "safety controls not evidenced")
    require(measurement["published_price_source"] == "https://modal.com/pricing"
            and measurement["published_price_observed_date"] == "2026-09-20"
            and measurement["hourly_rate_usd"] == RATES[gpu], "published price mismatch")
    ...
    for row in rows:
        require(row["sample_documents"] == 203 and row["sample_normalized_utf8_bytes"] == 999_753
                and row["training_steps"] == 256 and row["peak_gpu_memory_bytes"] > 0,
                "incomplete tokenizer probe")
        require(abs(row["model_and_optimizer_init_seconds"] + row["tokenization_seconds"]
                    + row["training_seconds"] - row["total_condition_seconds"]) < 0.02,
                "condition time mismatch")
    require([row["tokenizer"] for row in feasibility["records"]] == list(NAMES)
            and feasibility["conditions"] == 9, "feasibility cohort mismatch")
    seconds = 0.0
    for condition in feasibility["records"]:
        row = by_name[condition["tokenizer"]]
        per_seed = (row["model_and_optimizer_init_seconds"]
    ...
    require(billing["measurement_sha256"] == sha_file(measurement_path)
            and billing["measurement_content_sha256"] == measurement["content_sha256"],
            "billing is not bound to measurement")
    require(billing["modal_app_url"].startswith("https://modal.com/apps/")
            and billing["gpu"] == gpu and billing["app_metered_usage_usd"] >= 0,
            "missing Modal app charge")
    require(billing["published_price_source"] == "https://modal.com/pricing"
            and billing["published_price_observed_date"] == "2026-09-20"
            and billing["published_hourly_rate_usd"] == RATES[gpu], "billing price mismatch")
    before, after = billing["workspace_before"], billing["workspace_after"]
    for snapshot in (before, after):
        require(snapshot["hard_usage_limit_usd"] == 30.0
                and snapshot["post_credit_spend_limit_usd"] == 13.0
                and snapshot["charges_usd"] == 0.0, "workspace limit changed")
        close(snapshot["remaining_to_hard_limit_usd"], 30.0 - snapshot["usage_usd"] + 0.0,
              "remaining budget")
    ...
    return {"gpu": gpu, "status": "VERIFIED", "decision": decision,
            "measurement_file_sha256": sha_file(measurement_path),
            "billing_file_sha256": sha_file(billing_path),
            "sample_documents": 203, "sample_normalized_utf8_bytes": 999_753,
            "steps_per_tokenizer": 256, "measured_wall_seconds": measurement["wall_seconds"],
...
def verify(t4, t4_billing, a100, a100_billing, feasibility_path, commit):
    ...
    records = [verify_one("T4", t4, t4_billing, feasibility, commit),
               verify_one("A100-40GB", a100, a100_billing, feasibility, commit)]
    return {"schema_version": 1, "status": "INDEPENDENT_VERIFICATION_COMPLETE",
            "phase_c_conditions_started": 0, "validation_scored": False, "test_split_opened": False,
            "launch_authorized": False, "calibration_commit": commit,
            "feasibility_receipt_sha256": FEASIBILITY_HASH, "records": records}
...
def main():
    ...
    result = verify(args.t4, args.t4_billing, args.a100, args.a100_billing,
                    args.feasibility, args.commit)
