...
def reference_schedule(counts, vocab, regime, budget, context=128):
    ...
    return {"training_steps": steps, "training_target_tokens": targets,
            ...
            "core_analytical_flops": core, "output_projection_flops": output,
...
def verify(report_path, expected_sha, snapshot, output_path):
    ...
    for index, (record, selected) in enumerate(zip(report["conditions"], selection["conditions"])):
        h.require(h.read_json(report_path.parent / f"condition-{index:03d}.json") == record, "condition evidence mismatch")
        h.require((record["tokenizer"], record["vocab_budget"]) == (selected["tokenizer"], selected["vocab_budget"]),
                  "condition assignment mismatch")
        h.require(h.artifact_hashes(phase_a.parent / selected["artifact"]) == selected["artifact_hashes"],
        ...
        parameters = {"core_params": core_params, "non_embedding_params": core_params,
                      "input_embedding_params": 128 * vocab, "output_head_params": 128 * vocab,
        ...
        h.require(record["tokenizer_config"] == selected["tokenizer_config"] and
                  record["actual_vocab_size"] == selected["actual_vocab_size"] == vocab and
                  record["special_tokens"] == selection["special_tokens"], "tokenizer config/budget mismatch")
        ...
        for token, document in zip(token_rows, exposure["ordered_documents"]):
            h.require(all(token[field] == document[field] for field in
                          ("position", "id", "normalized_text_hash", "normalized_utf8_bytes", "source_utf8_bytes")),
        ...
        for regime in ("flops", "bytes"):
            ...
            h.require(all(original[key] == value for key, value in computed.items() if key != "fully_predicted_documents"),
            ...
            h.require(original["complete_document_bytes"] == h.byte_totals(sizes, count), "byte accounting disagreement")
            if regime == "bytes":
    ...
    result = {"verification_schema_version": 1, "accounting_verification": "PASS",
              "status": "preflight_rejected" if blocked or report["runtime_blockers"] or report["upstream_source_truncations_for_review"]
              else "preflight_feasible",
              "original_report_sha256": expected_sha, "verifier_sha256": h.file_hash(__file__),
              "preflight_implementation_sha256": h.file_hash(snapshot),
              "source_manifest_sha256": report["source_manifest_sha256"],
              "exposure_sha256": report["exposure_sha256"], "selection_sha256": report["selection_sha256"],
              "preflight_identity": report["preflight_identity"], "runtime_blockers": report["runtime_blockers"],
              "upstream_source_truncations_for_review": report["upstream_source_truncations_for_review"],
              "counter_boundary_notes": counter_notes, "conditions": rows,
              "fully_predicted_coverage_blockers": [{"tokenizer": r["tokenizer"], "vocab_budget": r["vocab_budget"]} for r in blocked],
              "original_evidence_unchanged": True, "tokenizers_reexecuted": False,
              "test_data_opened": False, "authorized_for_lm_execution": False}
    check_inputs()
