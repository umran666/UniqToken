...
def validate_record(row, name):
    ...
    h.require(
        row["documents"] == 42_897 and row["normalized_utf8_bytes"] == phase_c.EXPOSURE_BYTES,
        "feasibility exposure mismatch",
    )
    h.require(row["training_steps"] > 0 and row["training_target_tokens"] > 0, "empty feasibility accounting")
    h.require(
        row["actual_analytical_flops"] == row["core_analytical_flops"] + row["output_projection_flops"],
        "FLOP decomposition mismatch",
    )
    h.digest(row)
    return row
...
def run(args):
    ...
    for index, name in enumerate(phase_c.NAMES):
        ...
        baseline = next(
            ...
            if (row["tokenizer"], row["vocab_budget"], row["budget_regime"]) == (name, phase_c.VOCAB, "bytes")
        )
        ...
        record = {
            ...
            "encoding_workers": args.workers,
            **accounting,
            "tokenizer_preflight_seconds": time.monotonic() - started,
    ...
    body = {
        ...
        "pricing": {
            ...
            "hourly_components_usd": components,
            "total_per_hour_usd": per_hour,
        },
        ...
        "provenance": plan["provenance"],
        "runner_commit": plan["identity"]["commit_hash"],
