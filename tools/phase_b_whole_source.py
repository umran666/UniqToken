...
def restored_record(row, text):
    ...
    base.require(
        text.startswith(row["text"]) if truncated else text == row["text"],
    ...
    new["whole_record_provenance"] = {
        "original_training_id": row["id"],
        "original_was_truncated": truncated,
...
def select_whole_tail(rows, quota, additions):
    ...
    return selected, {"tail_target": target, **dimensions, "candidate_ids": [r["id"] for r in eligible]}
...
def regenerate(args):
    ...
    base.publish(
        ...
        {
            "status": "rejected_until_complete",
            "policy": POLICY,
    ...
    try:
        ...
        for split, spec in old["splits"].items():
            ...
            base.require(
                path.is_relative_to(root) and base.file_hash(path) == spec["sha256"], "old split hash mismatch"
            )
        ...
        quotas = {
            (g["domain"], g["language"]): g["normalized_utf8_bytes"]
            for g in old["freeze"]["selection"]["groups"]
            if g["split"] == "train"
        }
        selected, reports = [], []
        ...
        for key, group in by_stratum.items():
            ...
            def additions():
                ...
                for local in paths:
                    ...
                    if source["dataset"] == freezer.MADLAD:
                        ...
                        license_entry = next(
                            s for s in inventory.values() if s["local_path"].endswith("/licenses.json")
                        )
                        license_path = root / license_entry["local_path"]
                        base.require(
                            base.file_hash(license_path) == license_entry["sha256"], "license allowlist hash mismatch"
                        )
                    ...
                    for text, provenance, index in iterator:
                        if index <= last_historical or not text.strip() or freezer.RESERVED_CORPUS_TEXT.search(text):
                            continue
                        row = freezer.source_record(
                            text,
                            identifier=f"whole:{source['dataset']}:{source['remote_path']}:{index}",
                            language=key[1],
                            domain=key[0],
                            source=provenance,
                            source_record=index,
            ...
            base.require(
                all(not freezer.RESERVED_CORPUS_TEXT.search(r["text"]) for r in chosen),
        ...
        base.publish(
            output / "packing.json",
            {
                ...
                "strata": reports,
                "restored_records": [
                    r["whole_record_provenance"] | {"id": r["id"]}
                    for r in restored
                    if r["whole_record_provenance"]["original_was_truncated"]
                ],
            },
        )
        ...
        for local, entry in inventory.items():
            ...
            base.require(
                original.is_relative_to(root) and destination.resolve().is_relative_to(output),
        ...
        manifest = freezer.build_manifest(
            output, old["freeze"]["source_files"], new_splits, revisions=old["freeze"]["source_revisions"]
        )
        manifest["whole_record_regeneration"] = {
            "policy": POLICY,
            "historical_source_manifest_sha256": base.SOURCE_SHA,
        ...
        base.publish(
            output / "receipt.json",
            {
                "status": "whole_upstream_source_frozen",
        ...
        base.publish(
            ...
            {
                "status": "source_regeneration_rejected",
