...
def subset_indices(weights, target, snapshot_limit=MAX_SNAPSHOT_BYTES):
    ...
    base.require(
        len(weights) <= MAX_DOCUMENTS and all(type(w) is int and 0 < w <= target for w in weights),
        "invalid bounded candidate lengths",
    )
    # Include integer object headers, list pointers and transient full-width values.
    integer_bytes = (
        sys.getsizeof(0)
        + ((target + sys.int_info.bits_per_digit) // sys.int_info.bits_per_digit) * sys.int_info.sizeof_digit
    )
...
def solve_stratum(documents, quota, snapshot_limit=MAX_SNAPSHOT_BYTES):
    ...
    report = {
        ...
        "quota": quota,
        "rank_namespace": base.POLICY,
        "ranked_candidate_ids": [d["id"] for d in candidates],
    }
    if not fit:
    ...
    report.update(
        {
            "coverage_document_id": coverage["id"],
            "coverage_normalized_bytes": coverage["normalized_utf8_bytes"],
            "excluded_exceeds_residual_ids": [
                d["id"] for d in candidates if d["id"] != coverage["id"] and d["normalized_utf8_bytes"] > remaining
            ],
            "residual_candidate_ids": [d["id"] for d in residual],
            **dimensions,
        }
    )
    if chosen is None:
    ...
    report.update(
        ...
        selected_candidate_indices=chosen,
...
def solve_all(documents, allocation, snapshot_limit=MAX_SNAPSHOT_BYTES):
    ...
    for d in documents:
        base.require(
            type(d["normalized_utf8_bytes"]) is int and d["normalized_utf8_bytes"] > 0, "invalid document length"
        )
...
def verify_witness(parent, allocation, sample):
    ...
    for index, d in enumerate(docs):
        ...
        base.require(
            set(d) == set(original) | {"candidate_rank", "coverage_document", "position"}, "unexpected witness fields"
        )
        base.require(all(d[k] == v for k, v in original.items()), "altered document provenance/bytes")
        base.require(
            d["position"] == index and d["candidate_rank"] == base.ranked(original)["candidate_rank"],
            "rank/position mismatch",
        )
        groups[d["domain"], d["language"]].append(d)
    ...
    for key, quota in allocation.items():
        ...
        base.require(
            all(d["coverage_document"] == (d["id"] == expected["id"]) for d in selected), "coverage flag mismatch"
        )
        g = next((g for g in sample["strata"] if (g["domain"], g["language"]) == key), None)
        base.require(
            g is not None and g["packing_status"] == g["exact_quota_status"] == "PASS", "stratum gate mismatch"
        )
        required = {
            "allocated_normalized_bytes": quota,
            "actual_normalized_bytes": quota,
            "unused_quota_bytes": 0,
            "allocated_percent_of_cap": 100 * quota / sum(allocation.values()),
            "achieved_percent_of_actual": 100 * quota / sum(allocation.values()),
            "minimum_documents": 1,
            "minimum_normalized_bytes": (quota * 9 + 9) // 10,
            "selected_documents": len(selected),
            "eligible_documents": len(parent_groups[key]),
            "eligible_normalized_bytes": sum(d["normalized_utf8_bytes"] for d in parent_groups[key]),
            "skipped_documents": len(parent_groups[key]) - len(selected),
            "coverage_document_id": expected["id"],
            "source_utf8_bytes": sum(d["source_utf8_bytes"] for d in selected),
            "exhaustion_reason": "quota_filled",
            "ordered_document_ids": [d["id"] for d in selected],
            "global_positions": [d["position"] for d in selected],
            "last_global_position": selected[-1]["position"],
            "order_rule": "R",
            "early_exhaustion_allowed": True,
            "coverage_order_rank": base.digest(
                {"policy": base.POLICY, "purpose": "coverage_order", "domain": key[0], "language": key[1]}
            ),
            "ordered_list_sha256": base.digest(
                [
    ...
    domain_keys = {
        domain: sorted(
            (k for k in allocation if k[0] == domain),
            key=lambda k: (
                base.digest({"policy": base.POLICY, "purpose": "coverage_order", "domain": k[0], "language": k[1]}),
                k[1],
            ),
        )
    ...
    coverage_ids = [
        coverage[domain_keys[domain][i]]
        for i in range(max(map(len, domain_keys.values())))
        for domain in domains
        if i < len(domain_keys[domain])
    ]
    ...
    queues = {
        k: sorted((d for d in groups[k] if d["id"] != coverage[k]), key=lambda d: (d["candidate_rank"], d["id"]))
        for k in allocation
    }
    ...
    base.require(
        sample["normalized_utf8_bytes"] == sum(allocation.values()) == sum(d["normalized_utf8_bytes"] for d in docs),
        "total exact byte mismatch",
    )
    base.require(sample["source_utf8_bytes"] == sum(d["source_utf8_bytes"] for d in docs), "source-byte total mismatch")
    base.require(
        sample["selected_documents"] == len(docs) and sample["ordered_exposure_hash"] == base.digest(docs),
        "sample hash/count mismatch",
    )
...
def worker(attempt, mode):
    ...
    sample, reports = solve_all(
        documents if mode == "solve" else list(reversed(documents)), base.quotas(), plan["snapshot_limit_bytes"]
    )
    if mode == "solve":
        ...
        base.require(
            candidate["provenance"] == context and candidate["policy_sha256"] == plan["policy_sha256"],
            "candidate provenance mismatch",
        )
        verification = verify_witness(documents, base.quotas(), candidate["sample"])
        base.require(
            sample is not None and sample == candidate["sample"] and reports == candidate["search"],
            "independent-process deterministic witness mismatch",
        )
        base.require(
            base.file_hash(attempt / "candidate.json") == candidate_hash, "candidate changed during verification"
        )
        base.publish(
            attempt / "verification.json",
            {
                ...
                "checks": verification,
                "fresh_process_source_reload": True,
                "include_first_reconstruction_repeated": True,
                "input_order_reversed_for_check": True,
            },
        )
    check_pins(plan)
...
def rejection(attempt, reason, detail=""):
    ...
    all_rows = [
        known.get(key, {"domain": key[0], "language": key[1], "status": "NOT_COMPLETED"})
        for key in sorted(base.quotas())
    ]
    receipt = {
        "status": "exposure_rejected",
        "reason": reason,
        "detail": detail,
        "final_exposure_sha256": None,
        "plan_sha256": base.file_hash(attempt / "plan.json"),
        "strata": all_rows,
        "tokenizer_flop_preflight_run": False,
        "lm_training_run": False,
    }
...
def run_child(attempt, mode, deadline):
    ...
    with (attempt / f"{mode}.log").open("x", encoding="utf-8") as log:
        child = subprocess.Popen(
            [sys.executable, "-m", "tools.phase_b_exact_exposure", "worker", "--attempt", str(attempt), "--mode", mode],
            cwd=base.ROOT,
            stdout=log,
...
def publish_success(attempt, plan):
    ...
    base.require(
        ...
        and verification["candidate_sha256"] == base.file_hash(attempt / "candidate.json"),
        "missing or stale independent verification",
    )
    base.require(base.read_json(attempt / "search.json")["all_exact"] is True, "incomplete search")
    base.require(
        len(candidate["search"]) == len(base.quotas()) == len(candidate["sample"]["strata"])
        and {(r["domain"], r["language"]) for r in candidate["search"]} == set(base.quotas())
        and all(r["status"] == "EXACT_SOLUTION" for r in candidate["search"]),
        "incomplete stratum solutions",
    )
    body = {
    ...
    sample = candidate["sample"]
    receipt = {
        "status": "exact_packing_feasible",
        "final_exposure_sha256": manifest_hash,
        "policy_sha256": plan["policy_sha256"],
        "plan_sha256": base.file_hash(attempt / "plan.json"),
        "selection_sha256": base.SELECTION_SHA,
        "source_manifest_sha256": candidate["provenance"]["source_manifest_sha256"],
        ...
        "strata_exact": len(sample["strata"]),
...
def supervise(attempt, plan, child_runner=run_child):
    ...
    try:
        ...
        if not search["all_exact"]:
            reason = (
                "NO_EXACT_SOLUTION"
                if all(r["status"] in ("EXACT_SOLUTION", FAIL_NO_SOLUTION) for r in search["strata"])
                else "SEARCH_INCOMPLETE_RESOURCE_LIMIT"
            )
...
def generate(args):
    ...
    base.publish(
        attempt / "initial-rejection-receipt.json",
        {
            "status": "exposure_rejected",
            "reason": "SEARCH_NOT_COMPLETED",
            "final_exposure_sha256": None,
            "rule": "Unless a fully verified frozen/ bundle exists, this attempt is rejected. Hard termination or host shutdown cannot imply success.",
        },
    )
    try:
        ...
        pinned = {
            str(Path(p).resolve()): base.file_hash(p)
            for p in (args.selection, args.source, args.policy, __file__, base.__file__)
        }
        ...
        policy = {
            ...
            "rank_namespace": base.POLICY,
            "required_normalized_bytes": base.EXACT_BYTES,
            "quotas": [{"domain": k[0], "language": k[1], "bytes": v} for k, v in sorted(base.quotas().items())],
            "coverage_rule": "shortest_fitting_then_rank_then_ID_unchanged_v1",
            "solver": "suffix_bitset_zero_one_include_first",
            "ordering_rule": "R_unchanged_v1",
        ...
        plan = {
            ...
            "input_hashes": pinned,
            "policy_sha256": base.file_hash(attempt / "policy.json"),
            "timeout_seconds": MAX_SECONDS,
            "snapshot_limit_bytes": MAX_SNAPSHOT_BYTES,
            "generator": {
                "base_git_commit": subprocess.check_output(
                    ["git", "rev-parse", "HEAD"], cwd=base.ROOT, text=True
                ).strip(),
                "working_tree_dirty": bool(
                    subprocess.check_output(["git", "status", "--porcelain"], cwd=base.ROOT, text=True).strip()
                ),
                "source_hashes": {
                    str(Path(p).relative_to(base.ROOT)): base.file_hash(p)
                    for p in (Path(__file__).resolve(), Path(base.__file__).resolve())
                },
                "python": sys.version,
                "unicode_database_version": base.unicodedata.unidata_version,
        ...
        base.publish(
            attempt / "setup-rejection-receipt.json",
            {
                "status": "exposure_rejected",
